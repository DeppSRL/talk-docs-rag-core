...
# **identica** (vedi `talk_docs_rag_core.eval.judge`). Serve a rendere visibile la differenza fra un tasso di
...
def _fetch_chunks(cfg: RagConfig, chunk_ids: list[str]) -> dict[str, dict]:
    ...
    if not chunk_ids:
        return {}
    from talk_docs_rag_core.wiring import build_chroma_client
